(() => {
  ...
  function drawBackground() {
    ...
    g.addColorStop(0.45, '#1c103a');
    g.addColorStop(1, '#090514');
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, W(), H());

    const pulse = 0.18 + Math.sin(game.time * 2.1) * 0.09;
    ctx.globalAlpha = pulse + game.stage * 0.02;
    ctx.strokeStyle = '#7a33f8';
    ctx.lineWidth = 1;
    ...
    for (let i = 0; i < 4; i++) {
      ctx.strokeStyle = i % 2 ? '#ff2f67' : '#8f3aff';
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(cx, cy, 95 + i * 72 + Math.sin(game.time + i) * 12, 0, Math.PI * 2);
      ctx.stroke();
    }
    ctx.globalAlpha = 0.2;
    ctx.fillStyle = '#ff2d60';
    for (let i = 0; i < 8; i++) {
      const a = game.time * 0.26 + (Math.PI * 2 * i) / 8;
      ctx.beginPath();
      ctx.moveTo(cx, cy);
      ctx.lineTo(cx + Math.cos(a - 0.06) * 560, cy + Math.sin(a - 0.06) * 560);
      ctx.lineTo(cx + Math.cos(a + 0.06) * 560, cy + Math.sin(a + 0.06) * 560);
      ctx.closePath();
      ctx.fill();
    }
  ...
  function draw() {
    ...
    for (const e of game.enemies) {
      ...
      if (e.type === 'boss') {
        for (let k = 0; k < 8; k++) {
          const a = game.time * 0.9 + k * (Math.PI * 2 / 8);
          const rr = 46 + Math.sin(game.time * 2 + k) * 6;
          ctx.fillStyle = '#14080f';
          ctx.strokeStyle = '#ff2f67';
          ctx.lineWidth = 2;
          ctx.beginPath();
          ctx.moveTo(e.x + Math.cos(a) * rr, e.y + Math.sin(a) * rr);
          ctx.lineTo(e.x + Math.cos(a + 0.1) * (rr + 26), e.y + Math.sin(a + 0.1) * (rr + 26));
          ctx.lineTo(e.x + Math.cos(a - 0.1) * (rr + 26), e.y + Math.sin(a - 0.1) * (rr + 26));
          ctx.closePath();
          ctx.fill();
          ctx.stroke();
        ...
        ctx.fillStyle = '#fff6';
        ctx.fillRect(e.x - 120, e.y - 74, 240, 9);
        ctx.fillStyle = '#ff3f6c';
        ctx.fillRect(e.x - 120, e.y - 74, 240 * (e.hp / e.maxHp), 9);
      }
      ctx.globalAlpha = 1;
      e.hit *= 0.82;
    }
